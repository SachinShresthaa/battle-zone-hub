...
if ($_SERVER["REQUEST_METHOD"] == "POST" && empty($errorMessage)) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM ff_team_registration WHERE team_name = ? AND tournament_id = ?");
    ...
    if ($teamCount > 0) {
        ...
        header("Location: khalti/checkout.php?tournament_id=$tournament_id&entry_fee=" . urlencode($entryFee));
        exit();
    }
}
