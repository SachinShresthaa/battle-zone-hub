...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("SELECT kills, points, total_points FROM pubg_team_registration WHERE id = ? AND tournament_id = ?");
    ...
    // Update team data
    $updateStmt = $conn->prepare("UPDATE pubg_team_registration SET kills = ?, points = ?, total_points = ? WHERE id = ? AND tournament_id = ?");
    $updateStmt->bind_param("iiiii", $new_kills, $new_points, $new_total_points, $team_id, $latest_tournament_id);
    if ($updateStmt->execute()) {
         //echo "<script>alert('Team scores updated successfully!'); window.location.href=pubgleaderboard.php;</script>";
    } else {
...
<div class="container">
    ...
    <table>
        ...
        $stmt = $conn->prepare("SELECT id, team_name, kills, points, total_points FROM pubg_team_registration WHERE tournament_id = ? ORDER BY points DESC, kills DESC");
